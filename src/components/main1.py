...
def indexlist():
    ...
    if request.method == 'POST':
        if(result.form['get_options'] == False):
            ...
            sz3 = top_level[compressor_id] #we determined that "sz3" is the right string here by stripping out "pressio/" from the entries in children
            module_slots = [k for k in sz3.keys() if k.startswith(compressor_id)]

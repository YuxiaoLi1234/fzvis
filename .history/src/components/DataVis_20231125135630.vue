...
const colorControlPoints = [
    { value: max*255,  label:'max' },   // 蓝色
    { value: q3*255,  label:'75%'},  // 绿色
    { value: q2*255,  label:'median'}, // 黄色
    { value: q1*255,  label:'25%' }, // 橙色
    { value: min*255, label:'min' },  // 红色s
];
for(let i=0;i++;i<colorControlPoints.length){
    const min1 = colormap[0]
    const max1 = colormap[colormap.length-1]
    const r = min1.r + (i/colorControlPoints.length)*(max1.r-min1.r)
    const b = min1.b + (i/colorControlPoints.length)*(max1.b-min1.b)
    const g = min1.r + (i/colorControlPoints.length)*(max1.g-min1.g)
    let obj = {
        r:r,b:b,g:g
    }
    colorControlPoints[i][color] = obj
}
...
colorbarCanvas.addEventListener('mousedown', (e) => {
  ...
  if (clickY > colorbarHeight / 2 - 10 && clickY < colorbarHeight / 2 + 10) {
    ...
    drawControlPoints(); // 这是绘制所有控制点圆形的函数
  }

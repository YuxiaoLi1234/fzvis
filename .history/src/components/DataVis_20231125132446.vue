<template>
    <div id="data_vis">
        <div id="legendContainer"></div>
            ...
            <t-switch id='switch' size="large" v-model="slotChecked">
        <template v-slot:label="slotProps">{{ slotProps.value ? 'add point' : '关' }}</template>
      </t-switch>
...
// 添加鼠标事件监听器
// colorbarCanvas.addEventListener('click', function(e) {
//     var bbox = colorbarCanvas.getBoundingClientRect();
//     const clickX = (e.clientX - colorbarCanvas.getBoundingClientRect().left)* (colorbarCanvas.width / bbox.width)
//     const clickY = (e.clientY - colorbarCanvas.getBoundingClientRect().top)* (colorbarCanvas.height / bbox.height)
...
//     const newPoint = {
//       x: clickX,
//       y: colorbarHeight / 2, // 假设圆形应该在色条的垂直中心
//       radius: 7, // 圆的半径，可以根据需要进行调整
//       color: 'white' // 圆的颜色，可以根据需要进行调整
//     };

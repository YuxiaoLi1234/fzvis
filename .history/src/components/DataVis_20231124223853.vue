...
function onCanvasDblClick(e) {
  ...
  for (let i = 0; i < colorControlPoints.length; i++) {
    if (isPointInsideCircle(clickX, clickY, colorControlPoints[i])) {
      // 删除该圆
      clearCircle(colorControlPoints[i])
...
function clearCircle(point) {
  const clearRadius = point.radius + 1; // 小幅扩大清除区域以包括圆的边缘
  colorbarContext.save();
  colorbarContext.beginPath();
  colorbarContext.arc(point.x, point.y, clearRadius, 0, Math.PI * 2);
  colorbarContext.clip();
  colorbarContext.clearRect(point.x - clearRadius, point.y - clearRadius, clearRadius * 2, clearRadius * 2);

...
//     },
    drawd3:function(dataMin,dataMax,dataQ1,dataMedian,dataQ3){
     // 设置colorbar的宽度和高度
...
const svg = d3.select('#data_svg')
    ...
    .attr('height', colorbarHeight + 50); // 增加50像素的高度用于控制点拖动

// 控制点数据结构，初始设置为数据的最小值、25分位数、中位数、75分位数和最大值
const colorControlPoints = [
    { value: dataMin, color: { r: 0, g: 0, b: 255 } },   // 蓝色，数据的最小值
    { value: dataQ1, color: { r: 0, g: 255, b: 0 } },   // 绿色，25分位数
    { value: dataMedian, color: { r: 255, g: 255, b: 0 } }, // 黄色，中位数
    { value: dataQ3, color: { r: 255, g: 165, b: 0 } }, // 橙色，75分位数
    { value: dataMax, color: { r: 255, g: 0, b: 0 } },  // 红色，最大值
...
const colorbar = svg.append('rect')
    ...
    .attr('fill', generateColormap()); // 生成初始的colormap

// 创建控制点并绑定数据
const controlPoints = svg.selectAll('.control-point')
    .data(colorControlPoints)
    .enter().append('circle')
    .attr('class', 'control-point')
    .attr('cx', d => (d.value / 255) * colorbarWidth)
    .attr('cy', colorbarHeight / 2)
    .attr('r', 5)
    .style('fill', d => `rgb(${d.color.r}, ${d.color.g}, ${d.color.b})`)
    .call(d3.drag()
        .on('start', onMouseDown)
        .on('drag', onMouseMove)
        .on('end', onMouseUp));
console.log(colorbar,controlPoints)
// 数据可视化的位置
const dataVisualizationX = 50;
const dataVisualizationY = colorbarHeight + 20;

// 创建数据可视化矩形
const dataVisualization = svg.append('rect')
    .attr('x', dataVisualizationX)
    .attr('y', dataVisualizationY)
    .attr('width', colorbarWidth)
    .attr('height', colorbarHeight)
    .attr('fill', 'url(#colormapGradient)'); // 使用colormap的渐变填充

// 定义colormap的渐变
const colormapGradient = svg.append('linearGradient')
    .attr('id', 'colormapGradient')
    .attr('x1', '0%')
    .attr('y1', '0%')
    .attr('x2', '100%')
    .attr('y2', '0%');
...
// 更新colormap的渐变
function updateColormapGradient() {
    const stops = colorControlPoints.map(point => {
        const color = `rgb(${point.color.r}, ${point.color.g}, ${point.color.b})`;
        const position = (point.value / 255) * 100 + '%';
        return { color, position };
    });

    colormapGradient.selectAll('stop').remove(); // 移除现有的stop
    colormapGradient.selectAll('stop')
        .data(stops)
        .enter().append('stop')
        .attr('offset', d => d.position)
        .attr('stop-color', d => d.color);
}

// 鼠标事件处理函数
function onMouseDown() {
    d3.select(this).raise().classed('active', true); // 提高被点击的控制点
}

function onMouseMove(d) {
    if (d3.select(this).classed('active')) {
        const mouseX = d3.event.x;
        d.value = (mouseX / colorbarWidth) * 255;
        updateColormapGradient();
...
function updateDataVisualization() {
    ...
    dataVisualization.transition().attr('fill', colormap);
}

// 生成colormap的渐变
function generateColormap() {
    return `url(#colormapGradient)`;
}

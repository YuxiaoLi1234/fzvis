  ...
  export default {
    ...
    data(){
        return{
            ...
            parameters_copy:parameters.map((j)=>j[Object.keys(parameters[0])[i+1]]).map(item=>parseFloat(item)).filter((d)=>!d.includes(NaN)),
            svg:'',
            checked_c:[],
            compare_mode:false,
            option:{
    ...
    methods:{
        ...
        draw_levels:function(){
            ...
            this.option.tooltip.formatter = function (params) {
                    // const that = this
                    var htmlStr = '';
                    console.log('数据',that.parameters_copy)
                    for(var i=0;i<params.length;i++){
                        var param = params[i];
                        var xName = param.name;//x轴的名称
                        var seriesName = param.seriesName;//图例名称
                        var value = that.parameters_copy[i];//y轴值
                        var color = param.color;//图例颜色
                        if(i===0){
                        htmlStr += xName + '<br/>';//x轴的名称
                        }

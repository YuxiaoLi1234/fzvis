...
function clearArc(x, y, radius) {
  colorbarContext.globalCompositeOperation = 'destination-out';
  colorbarContext.beginPath();
  colorbarContext.arc(x, y, radius, 0, Math.PI * 2, true);
  colorbarContext.fill();
  colorbarContext.globalCompositeOperation = 'source-over'; // 恢复到默认值
}
function drawControlPoints() {
    // colorbarContext.cleararc(0, 0, canvas.width, canvas.height);
    clearArc()
    for (const point of colorControlPoints) {

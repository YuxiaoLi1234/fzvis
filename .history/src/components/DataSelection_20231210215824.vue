    ...
    update:function(){
        ...
        if(this.compressor_id.length == 0 || this.early_config.length == 0 || this.compressor_config.length == 0){
            ...
            'input_data':this.input_data
                ...
                let need1 = eval(response.data)
                console.log(eval(need1))
                that.compare_data['compressor_id'].push(need1['compressor_id'])
                that.compare_data['bound'].push(need1['bound'])
                that.compare_data['metrics'].push(need1['metrics'])
                document.getElementById('temp1').innerHTML=String(that.compare_data['compressor_id'])+'+'+String(that.compare_data['bound'])
                                                            +'+'+String(that.compare_data['metrics'])
                console.log(that.compare_data)
            })
            .catch((error)=>{
                console.log(error)
            })

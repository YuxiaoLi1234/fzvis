...
const controlPointGroup = svg
  ...
  .attr("transform", (d) => `translate(${d.x}, ${colorBarY - controlPointRadius * 2})`);
...
function dragged(event, d) {
  d.x = Math.max(colorBarX, Math.min(colorBarX + colorBarWidth, event.x));
  d3.select(this).attr("transform", `translate(${d.x}, ${colorBarY - controlPointRadius * 2})`);
...
function updateColormapAndColorBar() {
  controlPoints.sort((a, b) => a.x - b.x); // 根据控制点的x坐标排序
  const colorScale = d3.scaleLinear()
    .domain(controlPoints.map((d, i) => i / (controlPoints.length - 1))) // 根据控制点的位置设置域
    .range(controlPoints.map((d) => d.color));
...
gradient.selectAll("stop")
  .data(controlPoints)
  .enter().append("stop")
  .attr("offset", (d, i) => i * (1 / (controlPoints.length - 1)) * 100 + "%")

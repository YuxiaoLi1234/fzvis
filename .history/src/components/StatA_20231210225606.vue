  ...
  export default {
    ...
    mounted() {
        emitter.on('myEvent', (data) => {
                ...
                this.name = data.map(item=>item['compressor_id']).map((item)=>{
                    return {
                    value:item,
                    textStyle : {
                                    color: 'black'
                ...
                for(let i =0;i++;i<size){
                    let q = []
                    q.append(data['compressor_id'][i])
                    q.append(data['metrics'][i])
                    temp.append(q)
                }
                console.log('djankasnl',temp)
    ...
    methods:{
        ...
        draw_levels:function(){
            ...
            that.stats.forEach((item,i)=>{
                ...
                const u = normalize(parameters.map((j)=>j[Object.keys(parameters[0])[i+1]]).map(item=>parseFloat(item)))
                ...
                let obj = 
                    ...
                    data: u,
                    stack: that.name[i],
                    barWidth:String((1/u.length)*100)/3+'%',
            ...
            this.parameters_copy.forEach((item)=>{
                // console.log(item,va)
                const value = []
        ...
        draw:function(){
            ...
            console.log('djka',that.compare_data,that.name)
            this.draw_levels()
        },
        draw_parallel_c: function(data){
            ...
            let dimensions = Object.keys(data1[0]).slice(1);

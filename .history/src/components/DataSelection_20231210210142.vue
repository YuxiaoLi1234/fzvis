    ...
    update:function(){
        ...
        if(this.compressor_id.length == 0 || this.early_config.length == 0 || this.compressor_config.length == 0){
            ...
            'input_data':this.input_data
                ...
                let need1 = response.data['configration']
                this.compressor_data['compressor_id'].append(need1['compressor_id'])
                this.compressor_data['bound'].append(need1['bound'])
                this.compressor_data['metrics'].append(need1['metrics'])
                console.log(this.compressor_data)
            })
            .catch((error)=>{
                console.log(error)
            })

...
//     },
    drawd3:function(){
        // 创建一个100x100的随机数组（替换成您的实际数据）
const data = Array.from({ length: 100 }, () =>
  Array.from({ length: 100 }, () => Math.random())
);

// 创建颜色比例尺
const colorScale = d3.scaleSequential(d3.interpolateViridis) // 选择一种颜色插值方案，例如Viridis
  .domain([0, d3.max(data, d => d)]); // 根据数据的范围设置域
...
// 创建矩形用于绘制heatmap
const rectSize = width / data.length;
const heatmap = svg.selectAll("rect")
  .data(data.flat()) // 将二维数组扁平化
  .enter()
  .append("rect")
  .attr("x", (d, i) => (i % 100) * rectSize) // 计算x坐标
  .attr("y", (d, i) => Math.floor(i / 100) * rectSize) // 计算y坐标
  .attr("width", rectSize)
  .attr("height", rectSize)
  .attr("fill", d => colorScale(d)); // 根据数据值映射颜色
console.log(heatmap)
// 添加颜色条
...
gradient.selectAll("stop")
  ...
  .enter().append("stop")
  .attr("offset", d => d * 100 + "%")
  .attr("stop-color", d => colorScale(d3.max(data) * d)); // 根据数据的最大值生成渐变颜色

...
def indexlist():
    if request.method == 'POST':
        ...
        compressor_id = json.dumps(json.loads(request.data)['compressor_id'])
        early_config = json.dumps(json.loads(request.data)['early_config'])
        compressor_config = json.dumps(json.loads(request.data)['compressor_config'])
        print(early_config)
        ...
        e,stdout,stderr = ssh.exec_command('. /home/yli/lyx/spack/share/spack/setup-env.sh && spack load libpressio && python3 /home/yli/lyx/getdata.py {compressor_id} {early_config} {compressor_config}')
        output = stdout.read().decode('utf-8')
        error = stderr.read().decode('utf-8')
        print('gengxinlalala')
        # 处理输出
